...
export const IFrameHelper = {
  ...
  events: {
    ...
    onEvent({ eventIdentifier: eventName, data }) {
      if(eventName==="chatwoot:on-start-conversation"){
        isEvent=1;
      }

      if(isEvent===1 && eventName==="chatwoot:on-message"){
        ...
        if(name && email){
          ...
          window.$chatwoot.setConversationCustomAttributes({
            name:`Hyperce-${Math.ceil(Math.random() * 1000)}`,
            email:null,
            channel_id:channel_id
          })
        }
  ...
  onLoad: ({ widgetColor }) => {
    ...
    // if(email && name){
    //   window.$chatwoot.setUser(channel_id,{
    //     name:name ,
    //     email:email,
    //     avatar_url:" "
    //   });
